...
> ### 注意项：
> 该功能使用图形处理和键盘模拟方式实现，仅测试目前较常用的两个分辨率（1920*1080/2560*1440），其他分辨率可能无法正常使用（特别是16：9缩放比之外的分辨率），需要自行手动更改PickCards.json配置文件中的配置（截取W技能的大小和左上角坐标，通常就是一个QQ截图就完事了）；检测允许一定的滤镜范围，但建议使用该功能时不要开滤镜，以免导致某个检测无效。
